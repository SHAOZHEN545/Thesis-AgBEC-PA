...
Represent the source's incoming preparation by a normalizable reference packet

$$
|g\rangle=\int d\alpha\,g(\alpha)|\alpha\rangle ,
$$

where the coefficient $g(\alpha)$ is concentrated near the intended incoming asymptotic labels. Represent a detector's selected outgoing packet similarly by

$$
|h\rangle=\int d\beta\,h(\beta)|\beta\rangle .
$$

The symbols $|g\rangle$ and $|h\rangle$ specify free wave packets at an arbitrarily chosen reference time, here for convenience, $t=0$. 

Note: this does **not** mean that the source prepares $|g\rangle$ in the laboratory at $t=0$. It means that we use these reference packets to describe the preparation and detector at their actual times. Applying the familiar Schrödinger evolution rule with $H_0$ rather than $H$, the freely transported packets at preparation time $t_i$ and detection time $t_f$ are
...
"Freely transported" means evolved as the packets would move under $H_0$, without the scattering part $V$. Even when $t_i<0$, the first expression simply transports the reference packet backward from $0$ to $t_i$; no physical preparation at $0$ is implied. In the usual scattering setup, $V$ need not suddenly turn on at $t_i$. Rather, the incoming packet is far enough from the interaction region at a sufficiently early $t_i$ that its motion there is well approximated by free evolution. Taking $t_i\to-\infty$ makes this an asymptotic comparison with preparations farther and farther in the past.

The packet $e^{-iH_0t_f/\hbar}|h\rangle$ is the detector's target at $t_f$, not generally the state into which $|g\rangle$ would evolve if $V$ were absent. That no-interaction state would be $e^{-iH_0t_f/\hbar}|g\rangle$. We ask how much of the chosen detector packet appears after the incoming packet undergoes the **exact** evolution generated by $H$.
...
The factor $e^{iH_0t_f/\hbar}$ appears because the freely transported detector packet is used as a bra. As a check, if $V=0$ and hence $H=H_0$, the free factors cancel and $\mathcal A_{hg}(t_f,t_i)=\langle h|g\rangle$. The interaction changes this input-output overlap.
